...
aligned_categories_df = (
    gs.download_google_sheet(utils.AFS_GOOGLE_SHEET_ID, 'taxonomy_alignment')
...
processed_texts = pd.read_csv(utils.COMPANY_DESCRIPTIONS_PATH, names=['id', 'name', 'description'])
...
non_filtered_industries = (
    aligned_categories_df
    .query('need_filtering == 0 and source == "crunchbase_categories"')
    .category.to_list()
)
...
filtering_keywords = keywords_df.query('theme == "Filtering terms"').keywords_processed.to_list()
...
df_hits = (
    query_cb_categories(search_industries, CB, return_only_matches=True)
    .query("id in @filter_ok_ids")
)
...
df_keyword_hits = (
    Query.find_matches(keywords_list, return_only_matches=True)
    .query("id in @filter_ok_ids")
)
...
df_keywords_melt = (
    ...
    .merge(
        keywords_df_cats
        [['keywords_processed', 'theme', 'subtheme']]
        .astype({'keywords_processed': str}),
        left_on='variable', right_on='keywords_processed', how='left'
    )
    # clean up
    .drop(['variable', 'value'], axis=1)
    .drop_duplicates(['id', 'keywords_processed'])
    # turn the keywords back to list
    .assign(keywords_processed_list=lambda df: df.keywords_processed.apply(lambda x: literal_eval(x)))
)
...
df_keywords_melt_grouped.groupby(['theme','subtheme']).agg(counts=('id', 'count')).reset_index()
...
df_all_hits = pd.concat([df_hits_melt, df_keywords_melt_grouped.assign(source='crunchbase_keywords')])
...
childcare_candidates_filtered = (
    childcare_candidates
    .merge(CB.cb_organisations[['id', 'name', 'country', 'status']], how='left', on='id')
    .query('country in @utils.list_of_select_countries')
    .query('status != "closed"')
)

logging.info(f"Number of orgs after filtering by country: {len(childcare_candidates_filtered)}")

# Filter by funding
org_ids_with_funding = CB.get_funding_rounds(childcare_candidates_filtered)
childcare_candidates_filtered = (
    childcare_candidates_filtered
    .query('id in @org_ids_with_funding.org_id.to_list()')
)

logging.info(f"Number of orgs after filtering by funding: {len(childcare_candidates_filtered)}")
...
df_all_hits_filtered = df_all_hits.query("id in @childcare_candidates_filtered.id.to_list()")
...
combined_hits = pd.concat([
    initial_list_df_aligned,
    df_all_hits_filtered.merge(CB.cb_organisations[['id', 'name', 'homepage_url']], on='id', how='left'),
], ignore_index=True)
...
childcare_candidates_final_df = (
    ...
    # Add crunchbase data
    .merge(CB.cb_organisations[[
        'id', 'name', 'homepage_url', 'country', 'status',
        "cb_url",
        "country",
        "region",
        "city",
    ...
    .merge(processed_texts[['id', 'description']].rename(columns={'description': 'description_raw'}), how='left', on='id')
    .fillna('')    
    .reset_index()
)
...
childcare_candidates_final_df = childcare_candidates_final_df.merge(
    childcare_candidate_investors.rename(columns={'org_id': 'id'}),
    how='left', 
    on='id'
)
...
funding_rounds = CB.get_funding_rounds(
    (childcare_candidates_final_df[childcare_candidates_final_df.id.notnull()].drop_duplicates("id")),
    org_id_column="id",
)
...
company_list_funding = (
    childcare_candidates_final_df
    .merge(utils.get_last_rounds(funding_rounds), on="cb_id", how="left")
    ...
    .merge(initial_list_df[["company_name", "homepage_url", "relevancy_comment", "other_comments", "relevant"]], on=["company_name", "homepage_url"], how="left")
    # drop null rows
...
logging.info(f"Number of orgs after filtering by description length: {len(df_companies)}")
...
# %%
optimal_kmeans_params = cluster_analysis_utils.highest_silhouette_model_params(kmeans_search_results)
...
optimal_labels = cluster_analysis_utils.kmeans_clustering(childcare_vectors.vectors, param_dict)
...
optimal_cluster_keywords = cluster_analysis_utils.cluster_keywords(
    df_companies.description_processed.to_list(),
    df_companies.cluster_label.to_list()
)

df_companies['cluster_keywords'] = df_companies.cluster_label.map(optimal_cluster_keywords)
...
df_companies['cluster_relevance']= df_companies.cluster_label.astype(str).map(quick_validation)

# %%
cluster_label_valid_df['cluster_relevance'] = cluster_label_valid_df.cluster_label.astype(str).map(quick_validation)
cluster_label_valid_df
...
vectors_2d, fig = cluster_analysis_utils.cluster_visualisation(
    ...
    extra_data=(
        df_companies[['company_name', 'description_raw', 'company_keywords', 'sources', 'category', 'subthemes', 'homepage_url']]
...
vectors_2d, fig = cluster_analysis_utils.cluster_visualisation(
    # childcare_vectors.vectors,
    vectors_2d,
   (df_companies['cluster_label'].astype(str) + ': ' + df_companies['cluster_keywords'].astype(str)).to_list(),
    # Add short abstracts to the visualisation 
    extra_data=(
        df_companies[['cluster_keywords', 'cluster_relevance', 'company_name', 'description_raw', 'company_keywords', 'sources', 'category', 'subthemes', 'homepage_url', 'cb_url']]
        # .assign(abstract=lambda df: df.abstract.apply(lambda x: str(x)[0:300] + '...'))
    ),
)
...
# %%
company_list_funding_clustered = (
    company_list_funding
    .merge(
