...
colour_map = dict(zip(taxonomy_df.subtheme_full, taxonomy_df.colour_theme))
...
# Assume df is your DataFrame and 'x', 'y' are your columns
df = childcare_embeddings_df.rename(columns={"umap_1": "x", "umap_2": "y"})

# Define the Polygon (sector) in which you're interested
# This could be any shape, but here we define a triangle
sector = Polygon([(4.5, -2.5), (8, -1), (8, -5)])

# Create a mask where True indicates the point is inside the sector
mask = df.apply(lambda row: sector.contains(Point(row["x"], row["y"])), axis=1)

# Filter the DataFrame using the mask
df_sector = df[mask]

# Now df_sector contains only the points inside the sector
# You can export it to a CSV file
# df_sector.to_csv('sector_points.csv', index=False)

# Plot to visualize
fig, ax = plt.subplots()
df.plot.scatter("x", "y", color="blue", ax=ax)
df_sector.plot.scatter("x", "y", color="red", ax=ax)

plt.show()

# +
# Take care of product segment
subthemes_to_replace = [
    "<family_support:_general>",
    "<other>",
]

df_to_replace = df_sector[["cb_id", "model_subthemes"]].query(
    "model_subthemes in @subthemes_to_replace"
)
# -

# replace model_subthemes for companies in df_to_replace with a new subtheme
subthemes_df.loc[
    (subthemes_df.cb_id.isin(df_to_replace.cb_id))
    & (subthemes_df.model_subthemes.isin(subthemes_to_replace)),
    "model_subthemes",
] = "<family_support:_products>"
...
subthemes_df.model_subthemes.value_counts()
...
# save html
fig.save(
    str(PROJECT_DIR / "outputs/2023_childcare/figures/childcare_umap_themes.html"),
)
