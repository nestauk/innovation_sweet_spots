...
def define_context(sentence, options, n_words=None, search_term=None, nlp_model=None):
    """
...
def agg_categories(article_categories: Iterator[str], top_n=10):
    ...
    """
    category_count = sorted(
        Counter(article_categories).items(), key=lambda x: x[1], reverse=True
    )
...
def calculate_sentiment(
    sentence_collection, search_term, context, n_words=None, nlp_model=None
):
    """
    ...
    """
    target_context = [
        define_context(sentence, context, n_words, search_term, nlp_model)
    ...
    context_sentiment = [
        iss.get_sentence_sentiment_modified(elem, vader_replacements)
        for elem in target_context
    ]
...
def calculate_positive_pmi(
    cooccurrence_matrix, doc_term_m, token_names, token_counts, search_term
):
    """
...
def get_related_terms(noun_chunks, pmi, token_names, token_counts, min_mentions):
    ...
    pruned_noun_chunks = [
        elem for elem in pruned_noun_chunks if token_counts.get(elem, 0) > min_mentions
    ]
...
def get_normalised_rank(
    cooccurrence_m, token_names, token_counts, search_term, threshold=1
):
    """
    ...
    normalised_rank = {
        name: ix / total_word_set for ix, name in enumerate(count_rank_items)
    }
...
def get_article_text_df(grouped_articles, tags, article_metadata):
    ...
    year_articles = year_articles.merge(
        article_metadata, left_on="id", right_on="id", how="left"
    )
...
def get_flat_sentence_mentions(search_term, sentence_collection):
    ...
    for year, sentences in sentence_collection_df.groupby("year"):
        sentences_with_term = sentences[
            sentences["sentence"].str.contains(combined_expr, regex=True)
        ]
...
def total_docs(article_text_df):
    ...
    """
    total_docs = {
        year: len(articles) for year, articles in article_text_df.groupby("year")
    }
    return total_docs


def agg_term_sentiments(
    term, term_sentences, context="n_words", n_words=5, nlp_model=None
):
    """
    ...
    for year in term_sentences[term]:
        ...
        sentiments = calculate_sentiment(sentences, term, context, n_words)
        sentiments["year"] = year
        sentiments_all_years.append(sentiments)

    sentiments_across_years_df = pd.concat(sentiments_all_years)
    agg_sentiments = sentiments_across_years_df.groupby("year").agg(
...
def tokenise_and_count(
    individual_sentences, nlp_model, mentions_threshold=3, token_range=(1, 3)
):
    """
...
def get_key_terms(
    ...
    key_terms, norm_rank = identify_related_terms(
        search_term, cm, dtm, tn, tc, noun_chunks
    )
...
def get_svo_triples(sentence_collection, search_term, nlp_model):
    ...
    for sent in sentence_collection:
        ...
        for svo in sent_svos:
            if set(search_term.split()).issubset(
                set([str(elem) for elem in svo.subject])
            ):
                svo_subject.append(svo)
            if set(search_term.split()).issubset(
                set([str(elem) for elem in svo.object])
            ):
