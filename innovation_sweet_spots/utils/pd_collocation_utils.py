...
def tokenise_and_count(individual_sentences, 
    ...
    cooccurrence_matrix, doc_term_matrix, token_names, token_counts = \
        get_ngrams(tokenised_sentences,token_range, min_mentions = mentions_threshold)
...
def get_related_terms(pmi, token_names, token_counts, min_mentions):
    ...
    """
    terms_above_thresh = [elem for elem in token_names if \
...
def identify_related_terms(search_term,
    ...
    if len(pmis):
        ...
        key_related_terms = get_related_terms(pmis, 
                                              token_names, 
                                              token_counts, 
                                              min_mentions = mentions_threshold)
    
        normalised_rank = get_normalised_rank(cooccurrence_matrix, 
                                              token_names, 
                                              token_counts, 
                                              search_term, 
...
def get_key_terms(search_term, sentence_collection, nlp_model, mentions_threshold = 2, 
                  coocc_threshold = 1, token_range = (1,3)):
    ...
    Returns:
        ...
        A dict mapping terms to their normalised rank.
    """
    cm, dtm, tn, tc = tokenise_and_count(sentence_collection, 
                                         nlp_model, 
                                         mentions_threshold,
                                         token_range)
    
    key_terms, norm_rank = identify_related_terms(search_term, 
                                                  cm, 
                                                  dtm, 
                                                  tn, 
                                                  tc,  
