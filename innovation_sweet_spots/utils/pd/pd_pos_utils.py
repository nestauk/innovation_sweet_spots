...
def noun_chunks_w_term(noun_chunks_dict, search_terms):
    ...
    for year, chunks in noun_chunks_dict.items():
        contain_term = [
            [elem for elem in chunks if term in elem] for term in search_terms
        ]
...
def match_patterns_across_years(
    sentence_dict, nlp_model, pattern, n_years=3, field="sentence"
):
    ...
    if len(agg_phrases[year]) == 0:
        ...
        for p in agg_phrases[year]:
            ...
            if year in grouped_sentences.groups:
                dpu.view_collocations_given_year(
                    grouped_sentences.get_group(year),
                    metadata_dict,
...
def view_phrase_sentences_period(
    time_period,
    agg_phrases,
